...
using PersonRepository = DDDNoEventSourcingOrOrm.Infrastructure.PersonRepository;
...
namespace DDDNoEventSourcingOrOrm
{
    class Program
    {
        ...
        private async Task RunDemo()
        {
            ...
            var petNames = $"{string.Join(", ", newPerson.Pets.Take(newPerson.Pets.Count - 1).Select(p => p.Name))}, and {newPerson.Pets.Last().Name}";
